...
author: "Isabela Lucas Bruxellas & Tony Liang & Xue Wang"
date: "07/04/2022"
output:
...
Table \@ref(tab:user_means_table) displays the means of the variables. 
...
Table \@ref(tab:user_maxima_table) displays the maxima of the variables. 
...
Table \@ref(tab:user_minima_table) displays the minima of the variables. 
...
Tables \@ref(tab:user_means_table) - \@ref(tab:user-minima-table) show that the means are 0.37 for STG and 0.46 for PEG, the maximums are 0.99 for STG and 0.93 for PEG, and minimums are 0 for STG and 0 for PEG.
...
Figure \@ref(fig:peg_stg_plot) shows a scatterplot of PEG against STG.

---

```{r peg_stg_plot, out.width="50%", fig.cap = "Plot of PEG and STG"}
...
```{r linear_regression, include=FALSE, echo=FALSE}
lm_spec <- linear_reg() %>%
        set_engine("lm") %>%
        set_mode("regression")
...
Figure \@ref(fig:lm-prediction-plot) shows the model of linear regression between the STG and PEG

---
```{r lm-prediction-plot, out.width="50%", fig.cap = "Plot of Linear Regression"}
knitr::include_graphics("../results/model/lm_predictions.png")
```
---

Looking at the slopes and coefficients from the Figure \@ref(fig:lm-prediction-plot) from each of the predictors, we can create our prediction model:
...
Tables \@ref(tab:lm-rmse-table) and \@ref(tab:lm-rmspe-table) show that the RMSE (of our training data) of linear regression is 0.249 and the RMSPE (of our testing data) of the linear regression is 0.281.

Figure \@ref(fig:lm-regression-plot) shows a visualization of our linear regression 

---
```{r lm-regression-plot, out.width="50%", fig.cap = "Plot of Linear Regression"}
# This is correct, fix the previous lm prediction 
knitr::include_graphics("../results/model/lm_predictions.png")
```
---
...
Tables \@ref(tab:knn-rmse-table) and Table \@ref(tab:knn-rmspe-table) show that the RMSE (of training data) of K-NN regression is 0.227 and the RMSPE (of testing data) of K-NN regression is 0.257.

Figure \@ref(fig:knn-regression-plot) shows a visualization of our K-NN regression.
...
This figure shows our K-NN regression model overlaid our testing data with STG on the x-axis and PEG on the y-axis.
